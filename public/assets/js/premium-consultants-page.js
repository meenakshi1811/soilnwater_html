document.addEventListener('DOMContentLoaded', function () {
  const pageRoot = document.getElementById('premiumConsultantsPageRoot');
  if (!pageRoot) return;

  const filterBar = document.getElementById('premiumConsultantsFilterBar');
  const vendorsGrid = document.getElementById('premiumConsultantsGrid');
  const searchFilter = document.getElementById('premiumConsultantsFilterSearch');
  const categoryFilter = document.getElementById('premiumConsultantsFilterCategory');
  const subcategoryFilter = document.getElementById('premiumConsultantsFilterSubcategory');
  const radiusFilter = document.getElementById('premiumConsultantsFilterRadius');
  const paymentFilter = document.getElementById('premiumConsultantsFilterPayment');
  const locationFilter = document.getElementById('premiumConsultantsFilterLocation');
  const ratingFilters = pageRoot.querySelectorAll('.premium-consultants-filter-rating');
  const applyFiltersBtn = document.getElementById('premiumConsultantsApplyFilters');
  const resetFiltersBtn = document.getElementById('premiumConsultantsResetFilters');
  const loadingText = document.getElementById('premiumConsultantsLoadingText');
  const summaryText = document.getElementById('premiumConsultantsSummaryText');
  const scrollSentinel = document.getElementById('premiumConsultantsScrollSentinel');
  ...
  function updateSummary(payload) {
    ...
    if (payload.total > 0) {
      summaryText.textContent = 'Showing 1 to ' + payload.loaded_to + ' of ' + payload.total + ' premium consultants';
  ...
  async function reloadVendorsFromStart(options) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to load premium consultants');
      }
  ...
  async function loadNextVendorsPage() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to load more premium consultants');
      }
