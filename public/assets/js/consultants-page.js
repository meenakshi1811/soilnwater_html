document.addEventListener('DOMContentLoaded', function () {
  const pageRoot = document.getElementById('consultantsPageRoot');
  if (!pageRoot) return;

  const filterBar = document.getElementById('consultantsFilterBar');
  const consultantsGrid = document.getElementById('consultantsGrid');
  const premiumSection = document.getElementById('consultantsPremiumSection');
  const premiumTrack = document.getElementById('consultantsPremiumTrack');
  const searchFilter = document.getElementById('consultantsMarketFilterSearch');
  const categoryFilter = document.getElementById('consultantsMarketFilterCategory');
  const subcategoryFilter = document.getElementById('consultantsMarketFilterSubcategory');
  const radiusFilter = document.getElementById('consultantsMarketFilterRadius');
  const premiumFilter = document.getElementById('consultantsMarketFilterPremium');
  const verifiedFilter = document.getElementById('consultantsMarketFilterVerified');
  const paymentFilter = document.getElementById('consultantsMarketFilterPayment');
  const locationFilter = document.getElementById('consultantsMarketFilterLocation');
  const ratingFilters = pageRoot.querySelectorAll('.consultants-market-filter-rating');
  const applyFiltersBtn = document.getElementById('consultantsMarketApplyFilters');
  const resetFiltersBtn = document.getElementById('consultantsMarketResetFilters');
  const sortFilter = document.getElementById('consultantsMarketSort');
  const tabButtons = pageRoot.querySelectorAll('[data-consultants-tab]');
  const viewButtons = pageRoot.querySelectorAll('[data-consultants-view]');
  const loadingText = document.getElementById('consultantsLoadingText');
  const summaryText = document.getElementById('consultantsSummaryText');
  const scrollSentinel = document.getElementById('consultantsScrollSentinel');
  const viewAllLink = document.getElementById('consultantsViewAllLink');
  const categoryCards = pageRoot.querySelectorAll('[data-consultants-category-id]');
  ...
  const premiumUrl = pageRoot.dataset.premiumUrl || '/consultants/premium';
  ...
  let nextPageUrl = consultantsGrid?.dataset.nextPageUrl || '';
  let isLoading = false;
  let debounceTimer;
  ...
  function buildPremiumConsultantsUrl() {
  ...
  function buildConsultantsUrl(pageUrl) {
  ...
  function syncViewClasses() {
    ...
    consultantsGrid.classList.toggle('is-list-view', activeView === 'list');
    viewButtons.forEach(function (button) {
      button.classList.toggle('is-active', button.dataset.consultantsView === activeView);
    });
  }

  function syncTabClasses() {
    tabButtons.forEach(function (button) {
      button.classList.toggle('is-active', button.dataset.consultantsTab === activeTab);
    });
  }

  function syncBrowserUrl() {
    const nextUrl = buildConsultantsUrl(indexUrl);
  ...
  function updateSummary(payload) {
    ...
    if (payload.total > 0) {
      if (isPreviewListing) {
        summaryText.textContent = 'Showing ' + payload.loaded_to + ' of ' + payload.total + ' consultants';
      } else {
  ...
  function syncViewAllLink() {
    ...
    viewAllLink.href = buildConsultantsUrl(listingsUrl);
  }
  ...
  async function reloadConsultantsFromStart(options) {
    const settings = options || {};

    if (!consultantsGrid || isLoading) return;

    isLoading = true;
    setLoadingState(true);

    try {
      const response = await fetch(buildConsultantsUrl(indexUrl), {
        headers: {
      ...
      consultantsGrid.innerHTML = payload.html || '';
      nextPageUrl = isPreviewListing ? '' : (payload.next_page_url || '');
      consultantsGrid.dataset.nextPageUrl = nextPageUrl;
      syncViewClasses();
  ...
  async function loadNextConsultantsPage() {
    if (!nextPageUrl || isLoading || !consultantsGrid) return;

    isLoading = true;
    setLoadingState(true);

    try {
      const response = await fetch(buildConsultantsUrl(nextPageUrl), {
        headers: {
          'X-Requested-With': 'XMLHttpRequest',
          'Accept': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error('Failed to load more consultants');
      }

      const payload = await response.json();
      const emptyState = consultantsGrid.querySelector('.vendors-empty-state');
      ...
      if (payload.html) {
        consultantsGrid.insertAdjacentHTML('beforeend', payload.html);
      }
  ...
  if (window.location.hash === '#consultantsAllSection') {
    document.getElementById('consultantsAllSection')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
  ...
  if (premiumFilter) {
    premiumFilter.addEventListener('change', function () {
      if (premiumFilter.checked) {
        window.location.href = buildPremiumConsultantsUrl();
        return;
      }
      scheduleReload(0);
    });
  ...
  tabButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      if (isLoading) return;
      activeTab = button.dataset.consultantsTab || 'all';
      syncTabClasses();
      reloadConsultantsFromStart();
    });
  });
  ...
  viewButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      activeView = button.dataset.consultantsView || 'grid';
      syncViewClasses();
      scheduleReload(0);
    });
  });
  ...
  categoryCards.forEach(function (card) {
    card.addEventListener('click', function (event) {
      ...
      categoryFilter.value = card.dataset.consultantsCategoryId || '';
      if (subcategoryFilter) {
        subcategoryFilter.dataset.selected = '';
      }
      populateSubcategories();
