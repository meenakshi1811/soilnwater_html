...
class Consultant extends Model
{
    ...
    {
        return $this->hasMany(ConsultantBranch::class)->orderByDesc('is_primary')->orderBy('branch_name');
    }

    public function bannerSlides(): HasMany
    {
        return $this->hasMany(ConsultantBannerSlide::class)->orderBy('sort_order');
    }
    ...
    public function consultantUrl(): string
    {
        return url('/consultant/'.$this->slug);
    }

    public static function generateUniqueSlug(string $name, ?int $exceptId = null): string
    {
        $base = Str::slug($name) ?: 'consultant';
        $slug = $base;
        $counter = 1;
