...
<div class="admin-panel ems-page">
  ...
  <div class="chart-card">
    <form method="POST" action="{{ $isEdit ? route('educator.materials.update', $material) : route('educator.materials.store') }}" enctype="multipart/form-data">
      @csrf
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          @if($material->file_name)<small class="text-muted">Current: {{ $material->file_name }}</small>@endif
        </div>
        <div class="col-md-6">
          <label class="form-label">Thumbnail</label>
          <input type="file" name="thumbnail" class="form-control" accept="image/*">
          @if($material->thumbnailUrl())<div class="mt-2"><img src="{{ $material->thumbnailUrl() }}" alt="" style="max-height:80px" class="rounded"></div>@endif
        </div>
        <div class="col-md-8">
      ...
      <div class="mt-4 d-flex gap-2">
        <button type="submit" class="btn btn-primary ems-btn-primary">{{ $isEdit ? 'Update & submit' : 'Submit for approval' }}</button>
