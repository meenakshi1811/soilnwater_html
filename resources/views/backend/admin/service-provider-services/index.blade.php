...
<div class="admin-panel ems-page">
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2"><div><p class="ems-kicker mb-1">Admin Portal</p><h2 class="admin-title mb-0">Consultation Service Approvals</h2></div><a href="{{ route('admin.service-provider-services.all.index') }}" class="btn btn-outline-primary">All Consultation Services</a></div>
...
(function () {
  ...
  function postAction(id, action) { fetch('/admin/service-provider-services/' + id + '/' + action, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]')?.content, 'Accept': 'application/json' } }).then(function (response) { if (!response.ok) throw new Error(); return response.json().catch(function () { return {}; }); }).then(function (data) { table.ajax.reload(null, false); notify('success', data.message || ('Consultation service ' + (action === 'approve' ? 'approved' : 'rejected') + '.')); }).catch(function () { notify('error', 'Unable to update consultation service.'); }); }
