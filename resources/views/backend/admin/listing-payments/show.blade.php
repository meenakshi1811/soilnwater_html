...
<div class="admin-panel ems-page">
    <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 mb-4">
        <div>
            ...
            <h2 class="admin-title mb-1">{{ $submission->listingTypeLabel() }} Payment Review</h2>
            <p class="text-muted mb-0">Verify the payment screenshot and activate this {{ strtolower($submission->listingTypeLabel()) }}.</p>
        </div>
        <div>
        ...
        <div class="col-lg-5">
            <div class="chart-card p-3 p-lg-4 h-100">
                ...
                <dl class="row mb-0 small">
                    ...
                    <dd class="col-sm-8">{{ $submission->listingTypeLabel() }}</dd>

                    <dt class="col-sm-4">Title</dt>
                    <dd class="col-sm-8">{{ $submission->listingDisplayName() }}</dd>

                    @if($submission->amount)
                        <dt class="col-sm-4">Amount</dt>
                        <dd class="col-sm-8">₹{{ number_format((float) $submission->amount, 2) }}</dd>
                    @endif
                ...
                @if($submission->isPending())
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        <button type="button" class="btn btn-success js-listing-payment-review" data-action="approve">
                            <i class="fa-solid fa-check me-1"></i> Verify &amp; Activate
                        </button>
                        <button type="button" class="btn btn-outline-danger js-listing-payment-review" data-action="reject">
...
<script>
(function () {
    const reviewButtons = document.querySelectorAll('.js-listing-payment-review');
    if (!reviewButtons.length) {
        return;
    }

    const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
    const approveUrl = @json(route('admin.listing-payments.approve', $submission));
    const rejectUrl = @json(route('admin.listing-payments.reject', $submission));
    ...
    reviewButtons.forEach((button) => {
        button.addEventListener('click', async () => {
            ...
            const result = await Swal.fire({
                title: action === 'approve' ? 'Verify payment and activate?' : 'Decline this payment proof?',
                text: action === 'approve'
                    ? 'This will activate the ad/offer immediately.'
                    : 'The user can submit a new screenshot after declining.',
                icon: 'question',
                showCancelButton: true,
                confirmButtonText: action === 'approve' ? 'Yes, activate' : 'Yes, decline',
