...
@php
    ...
    $formAction = $formAction ?? route('service_provider.public-page.update');
    $previewUrl = $previewUrl ?? route('service_provider.public-page.preview');
    $bannerDeleteBaseUrl = $bannerDeleteBaseUrl ?? url('service/banner-slides').'/';
...
<div class="admin-panel ems-page service_provider-public-live-editor">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <p class="ems-kicker mb-1">{{ $isAdmin ? 'Admin Panel' : 'Service Panel' }}</p>
            <h2 class="admin-title mb-0">{{ $service_provider->publicDisplayName() }}</h2>
            <p class="text-muted small mb-0">
                @if($isAdmin)
                    Edit this service page as admin. Publishing goes live immediately — no approval needed.
                @else
    ...
    <div class="alert {{ $service_provider->public_page_status === 'pending' ? 'alert-warning' : ($service_provider->public_page_status === 'approved' ? 'alert-success' : ($service_provider->public_page_status === 'declined' ? 'alert-danger' : 'alert-info')) }} py-2">
        ...
        @if($isAdmin)
            ...
            @endif
        @elseif($service_provider->public_page_status === 'pending')
    ...
    <form id="publicPageForm" method="POST" action="{{ $formAction }}" enctype="multipart/form-data" class="vendor-store-page service_provider-preview-edit-mode" data-banner-delete-url="{{ $bannerDeleteBaseUrl }}" data-card-placeholder-url="{{ asset('assets/images/vendor-card-placeholder.svg') }}">
        @csrf
        @method('PUT')
