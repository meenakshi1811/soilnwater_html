...
<div class="admin-panel ems-page">
  <div class="d-flex justify-content-between align-items-center mb-4"><div><p class="ems-kicker mb-1">Service Provider Portal</p><h2 class="admin-title mb-0">{{ $service->exists ? 'Edit Service' : 'Add Service' }}</h2></div><a href="{{ route('service_provider.services.index') }}" class="btn btn-outline-secondary">Back to Listing</a></div>
  @php
    ...
    $businessTypes = ['Architect', 'Lawyer', 'Landscaper', 'Software Service Provider', 'Business'];
  @endphp
  ...
  <form id="service_provider-service-form" data-ajax-create="{{ $service->exists ? '0' : '1' }}" method="POST" enctype="multipart/form-data" action="{{ $service->exists ? route('service_provider.services.update', $service) : route('service_provider.services.store') }}" class="row g-3">@csrf @if($service->exists) @method('PUT') @endif
    ...
    <div class="col-lg-4"><div class="chart-card p-4"><h5 class="mb-3">Media & Location</h5>
      <label class="form-label">Service Provider Image / Service Image</label><input class="form-control @error('image') is-invalid @enderror" type="file" name="image" accept="image/*">@error('image')<div id="image-error" class="invalid-feedback d-block">{{ $message }}</div>@enderror<small class="text-muted">Upload one image only. Max 4 MB.</small>
      ...
      @unless($service->exists)<div class="form-check mt-3"><input class="form-check-input @error('accept_terms') is-invalid @enderror" type="checkbox" value="1" id="accept_terms" name="accept_terms" required><label class="form-check-label" for="accept_terms">I accept the <a href="{{ route('frontend.terms.show', ['moduleKey' => 'service_providers']) }}" target="_blank" rel="noopener" class="fw-semibold text-decoration-underline" style="color:#0d6efd;">Terms & Conditions</a>.</label>@error('accept_terms')<div id="accept_terms-error" class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>@endunless
...
$(function () {
  ...
  }, 'Please add at least one service duration and price.');
  ...
  function applyServerErrors(errors) {
    ...
    Object.entries(errors || {}).forEach(function ([field, messages]) {
      ...
      if (normalizedField.startsWith('charge_duration') || normalizedField.startsWith('charge_price')) { showChargeError(message); return; }
      ...
      fetch(form.action, { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, body: new FormData(form) })
        .then(async function (res) {
          ...
          if (!res.ok) {
            ...
            notify('error', res.status === 422 ? 'Please fix the highlighted fields and try again.' : (payload.message || 'Unable to save service.'));
            return;
          }
