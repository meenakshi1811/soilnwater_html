        ...
        <form method="POST" action="{{ !empty($isEdit) ? route('ads.update', $ad) : route('ads.store', ['sizeType' => $sizeType, 'template' => $template->id??null]) }}" novalidate data-subcategory-url-base="{{ url('/dashboard/ads/categories') }}">
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="valid_until" class="form-control @error('valid_until') is-invalid @enderror" value="{{ old('valid_until', optional($ad->valid_until ?? null)->format('Y-m-d')) }}" min="{{ now()->toDateString() }}" {{ $sizeType === "square" ? "max=\"" . now()->addDays(6)->toDateString() . "\"" : "" }} required>
                    @error('valid_until')
