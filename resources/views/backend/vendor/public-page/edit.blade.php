...
@php
    ...
    $formAction = $formAction ?? route('vendor.public-page.update');
    $previewUrl = $previewUrl ?? route('vendor.public-page.preview');
    $bannerDeleteBaseUrl = $bannerDeleteBaseUrl ?? url('vendor/banner-slides').'/';
    $backUrl = $backUrl ?? null;
    $heroMainColor = old('hero_main_style.color', $vendor->hero_main_style['color'] ?? '#1b2b44');
    $heroSubColor = old('hero_sub_style.color', $vendor->hero_sub_style['color'] ?? '#1b2b44');
@endphp
<div class="admin-panel ems-page vendor-public-live-editor">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <p class="ems-kicker mb-1">{{ $isAdmin ? 'Admin Panel' : 'Vendor Panel' }}</p>
            <h2 class="admin-title mb-0">{{ $vendor->publicDisplayName() }}</h2>
            <p class="text-muted small mb-0">
                @if($isAdmin)
                    Edit this store page as admin. Publishing goes live immediately — no approval needed.
                @else
    ...
    <div class="alert {{ $vendor->public_page_status === 'pending' ? 'alert-warning' : ($vendor->public_page_status === 'approved' ? 'alert-success' : ($vendor->public_page_status === 'declined' ? 'alert-danger' : 'alert-info')) }} py-2">
        <strong>Page status:</strong> {{ ucfirst($vendor->public_page_status ?? 'draft') }}.
        @if($isAdmin)
            @if($vendor->public_page_status === 'approved')
    ...
    <form id="publicPageForm" method="POST" action="{{ $formAction }}" enctype="multipart/form-data" class="vendor-store-page vendor-preview-edit-mode" data-banner-delete-url="{{ $bannerDeleteBaseUrl }}" data-card-placeholder-url="{{ asset('assets/images/vendor-card-placeholder.svg') }}">
        @csrf
        @method('PUT')
