@if($post->isStudentCornerPost())
    <div class="chart-card p-3 p-lg-4 mb-4">
        <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3">
            <div>
                <h5 class="mb-1">Student Corner post settings</h5>
                <p class="text-muted small mb-0">Privacy, sharing, competition entry, and engagement summary.</p>
            </div>
        ...
        <div class="row g-3 small mb-3">
            <div class="col-md-3">
                <div class="border rounded p-3 bg-light h-100">
                    ...
                    <strong>{{ \App\Support\CommunityContentTaxonomy::studentCornerPublishAsOptions()[$post->resolvedPublishAs()] ?? $post->publishAsLabel() }}</strong>
                </div>
            </div>
            <div class="col-md-3">
                <div class="border rounded p-3 bg-light h-100">
                    <div class="text-muted mb-1">Visibility</div>
                    <strong>{{ $post->studentCornerVisibilityLabel() }}</strong>
                </div>
            </div>
            <div class="col-md-3">
                <div class="border rounded p-3 bg-light h-100">
                    <div class="text-muted mb-1">Competition</div>
                    <strong>{{ data_get($post->meta, 'student_corner_submit_to_competition') ? 'Submitted' : 'Not submitted' }}</strong>
                </div>
            </div>
            <div class="col-md-3">
                <div class="border rounded p-3 bg-light h-100">
        ...
        @if($post->requiresStudentCornerPrivateLink() && filled($post->studentCornerPrivateLinkUrl()))
            <div class="alert alert-info py-2 px-3 small mb-3">
                <strong>Private link:</strong>
                <div class="input-group input-group-sm mt-2">
                    <input type="text" class="form-control" value="{{ $post->studentCornerPrivateLinkUrl() }}" readonly id="studentCornerManagePrivateLink">
                    <button type="button" class="btn btn-outline-secondary" onclick="navigator.clipboard.writeText(document.getElementById('studentCornerManagePrivateLink').value)">Copy</button>
                </div>
            </div>
        @endif
        ...
        @if(filled(data_get($post->meta, 'student_corner_ask_community')))
            <div class="alert alert-primary py-2 px-3 small mb-3">
                <strong>Ask the community:</strong> {{ data_get($post->meta, 'student_corner_ask_community') }}
            </div>
        @endif
