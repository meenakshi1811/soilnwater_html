...
<div class="admin-panel ems-page">
  ...
  @php
    ...
    $storedCharges = collect($service->consultation_charges ?: [])->map(function ($charge, $key) {
        return is_array($charge) ? ['duration' => $charge['duration'] ?? '', 'price' => $charge['price'] ?? '', 'note' => $charge['note'] ?? ''] : ['duration' => $key, 'price' => $charge, 'note' => ''];
...
document.getElementById('add-charge-tier')?.addEventListener('click', function () {
  ...
  row.innerHTML = '<div class="row g-2 align-items-end"><div class="col-md-5"><label class="form-label">Duration</label><select class="form-select charge-duration-select" name="charge_duration[]"><option value="">Select duration</option>' + options + '</select></div><div class="col-md-5"><label class="form-label">Price ₹</label><input type="number" step="0.01" min="0" class="form-control charge-price-input" name="charge_price[]" placeholder="Price"><label class="form-label small text-muted mt-2 mb-1">Note (optional)</label><input type="text" class="form-control" name="charge_note[]" placeholder="Optional note for this price"></div><div class="col-md-2"><button type="button" class="btn btn-outline-danger btn-sm remove-charge-tier">Remove</button></div></div>';
...
$(function () {
  ...
  function applyServerErrors(errors) {
    ...
    Object.entries(errors || {}).forEach(function ([field, messages]) {
      ...
      if (normalizedField.startsWith('charge_duration') || normalizedField.startsWith('charge_price')) { mapped['charge_duration[]'] = message; return; }
  ...
  $form.validate({
    ...
    rules: {
      ...
      'charge_duration[]': { requireChargeRow: true },
      'charge_price[]': { number: true, min: 0 },
    ...
    messages: {
      ...
      'charge_duration[]': { requireChargeRow: 'Please add at least one consultation duration and price.' },
