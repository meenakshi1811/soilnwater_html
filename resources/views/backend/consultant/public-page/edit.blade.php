...
@php
    $consultantStoreCssVersion = file_exists(public_path('assets/css/vendor-store.css')) ? filemtime(public_path('assets/css/vendor-store.css')) : now()->timestamp;
    $consultantPortalCssVersion = file_exists(public_path('assets/css/vendor-portal.css')) ? filemtime(public_path('assets/css/vendor-portal.css')) : now()->timestamp;
    $consultantPublicPageJsVersion = file_exists(public_path('assets/js/vendor-public-page.js')) ? filemtime(public_path('assets/js/vendor-public-page.js')) : now()->timestamp;
@endphp

@push('styles')
<link rel="stylesheet" href="{{ asset('assets/css/vendor-store.css') }}?v={{ $consultantStoreCssVersion }}">
<link rel="stylesheet" href="{{ asset('assets/css/vendor-portal.css') }}?v={{ $consultantPortalCssVersion }}">
@endpush

@section('content')
@php
    $heroMainColor = old('hero_main_style.color', $consultant->hero_main_style['color'] ?? '#ffffff');
    $heroSubColor = old('hero_sub_style.color', $consultant->hero_sub_style['color'] ?? '#ffffff');
@endphp
<div class="admin-panel ems-page consultant-public-live-editor">
    ...
    <form id="publicPageForm" method="POST" action="{{ route('consultant.public-page.update') }}" enctype="multipart/form-data" class="vendor-store-page consultant-preview-edit-mode" data-banner-delete-url="{{ url('consultant/banner-slides') }}/" data-card-placeholder-url="{{ asset('assets/images/vendor-card-placeholder.svg') }}">
        ...
        <input type="text" name="hero_main_heading" value="{{ old('hero_main_heading', $consultant->hero_main_heading) }}" data-sync-input="hero-main" hidden>
        <input type="text" name="hero_sub_heading" value="{{ old('hero_sub_heading', $consultant->hero_sub_heading) }}" data-sync-input="hero-sub" hidden>
        <input type="text" name="display_name" value="{{ old('display_name', $consultant->display_name) }}" data-sync-input="display-name" hidden>
        <input type="text" name="phone" value="{{ old('phone', $consultant->phone) }}" data-sync-input="phone" hidden>
        <input type="email" name="email" value="{{ old('email', $consultant->email) }}" data-sync-input="email" hidden>
        <input type="text" name="city" value="{{ old('city', $consultant->city) }}" data-sync-input="city" hidden>
        <input type="text" name="address" value="{{ old('address', $consultant->address) }}" data-sync-input="address" hidden>
        <input type="url" name="facebook_url" value="{{ old('facebook_url', $consultant->facebook_url) }}" hidden>
        <input type="url" name="instagram_url" value="{{ old('instagram_url', $consultant->instagram_url) }}" hidden>
        <input type="hidden" name="hero_main_style[color]" data-style-input="hero-main" data-style-prop="color" value="{{ $heroMainColor }}">
        <input type="hidden" name="hero_main_style[fontSize]" data-style-input="hero-main" data-style-prop="fontSize" value="{{ old('hero_main_style.fontSize', $consultant->hero_main_style['fontSize'] ?? '') }}">
        <input type="hidden" name="hero_main_style[fontFamily]" data-style-input="hero-main" data-style-prop="fontFamily" value="{{ old('hero_main_style.fontFamily', $consultant->hero_main_style['fontFamily'] ?? '') }}">
        <input type="hidden" name="hero_main_style[fontWeight]" data-style-input="hero-main" data-style-prop="fontWeight" value="{{ old('hero_main_style.fontWeight', $consultant->hero_main_style['fontWeight'] ?? '') }}">
        <input type="hidden" name="hero_sub_style[color]" data-style-input="hero-sub" data-style-prop="color" value="{{ $heroSubColor }}">
        <input type="hidden" name="hero_sub_style[fontSize]" data-style-input="hero-sub" data-style-prop="fontSize" value="{{ old('hero_sub_style.fontSize', $consultant->hero_sub_style['fontSize'] ?? '') }}">
        <input type="hidden" name="hero_sub_style[fontFamily]" data-style-input="hero-sub" data-style-prop="fontFamily" value="{{ old('hero_sub_style.fontFamily', $consultant->hero_sub_style['fontFamily'] ?? '') }}">
        <input type="hidden" name="hero_sub_style[fontWeight]" data-style-input="hero-sub" data-style-prop="fontWeight" value="{{ old('hero_sub_style.fontWeight', $consultant->hero_sub_style['fontWeight'] ?? '') }}">

        <div class="consultant-editor-panel mb-3">
            <div class="consultant-editor-panel-head">
                <i class="fa-solid fa-user-tie text-primary"></i>
                <div>
                    <strong>Consultant header</strong>
                    <p class="mb-0 small text-muted">This matches your live consultant. Click the logo box to upload or change your logo.</p>
                </div>
            </div>
            <header class="vendor-store-header consultant-header-preview">
                <div class="container d-flex align-items-center justify-content-between flex-wrap gap-3 py-3">
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <label class="vendor-logo-dropzone mb-0" for="logoInput" title="Click to upload your consultant logo">
                            <span class="vendor-logo-dropzone-inner" id="logoPreviewWrap">
                                @if($consultant->logo)
                                    <img src="{{ asset($consultant->logo) }}" alt="Consultant logo" class="vendor-logo-dropzone-img" id="logoPreviewImg">
                                @else
                                ...
                                @endif
                                <span class="vendor-logo-dropzone-hint"><i class="fa-solid fa-camera"></i> {{ $consultant->logo ? 'Change' : 'Upload' }}</span>
                            </span>
                            <input type="file" name="logo" id="logoInput" class="d-none" accept="image/*">
                        </label>
                        <strong class="fs-4 mb-0 vendor-store-name-fallback {{ $consultant->logo ? 'd-none' : '' }}" id="storeNamePreview">{{ $consultant->publicDisplayName() }}</strong>
                    </div>
            ...
            <div class="vendor-store-url-row px-3 pb-3">
                <label class="form-label small text-muted mb-1">Your consultant link</label>
                <div class="input-group input-group-sm" style="max-width:560px; width:100%;">
                    <span class="input-group-text">{{ url('/consultant') }}/</span>
                    <input type="text" name="slug" class="form-control" value="{{ old('slug', $consultant->slug) }}" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="your-consultant-name">
                </div>
            </div>
        </div>

        <div class="consultant-editor-panel mb-4">
            <div class="consultant-editor-panel-head">
        ...
        <div class="consultant-form-card mb-4">
            ...
            <h1 class="vendor-live-editable" contenteditable="true" data-sync-target="hero-main" data-hero-editable="main" data-word-limit="500" style="@if(!empty($consultant->hero_main_style)){{ collect($consultant->hero_main_style)->filter(fn($v) => filled($v))->map(fn($v, $k) => \Illuminate\Support\Str::kebab($k).':'.$v)->implode(';') }}@endif">{{ old('hero_main_heading', $consultant->hero_main_heading ?: 'Your Main Heading') }}</h1>
            <div class="d-flex justify-content-end small text-muted mb-2" data-hero-word-counter="hero-main">0 / 500 words</div>
            <div id="heroSubHeadingEditor" class="vendor-live-editable vendor-lite-html-editor mt-2" contenteditable="true" data-sync-target="hero-sub" data-sync-html="1" data-hero-editable="sub" role="textbox" aria-label="Hero subheading" style="@if(!empty($consultant->hero_sub_style)){{ collect($consultant->hero_sub_style)->filter(fn($v) => filled($v))->map(fn($v, $k) => \Illuminate\Support\Str::kebab($k).':'.$v)->implode(';') }}@endif">{!! old('hero_sub_heading', $consultant->hero_sub_heading ?: 'Your sub heading appears here') !!}</div>
        </div>
        ...
        <div class="consultant-form-card mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    ...
                    <p class="text-muted small mb-0">This content is for a <strong>separate About Us page</strong>, not your consultant home page sections.</p>
                </div>
            </div>
            <div class="alert alert-warning small mb-3">
                <i class="fa-solid fa-circle-info me-1"></i>
                Visitors open this from Consultant Header <strong>About Us</strong> and it opens in a new page.
            </div>
            <div class="consultant-lite-editor" data-lite-editor="about-us">
                <div class="consultant-lite-editor-toolbar" aria-label="About Us formatting tools">
                ...
                <div id="consultantAboutUsEditor" class="vendor-lite-html-editor" contenteditable="true" data-lite-editor-target="description" role="textbox" aria-label="About Us page content">{!! old('description', $consultant->description) ?: '<p>Write your About Us content here...</p>' !!}</div>
                <textarea name="description" id="consultantAboutUsInput" class="d-none" rows="10">{{ old('description', $consultant->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="consultant-form-card mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    ...
                    <p class="text-muted small mb-0">Add your social URLs and see how they will appear on your public consultant page.</p>
                </div>
            </div>
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label class="form-label">Facebook URL</label>
                    <input type="url" class="form-control" data-social-input="facebook" value="{{ old('facebook_url', $consultant->facebook_url) }}" placeholder="https://facebook.com/yourpage">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Instagram URL</label>
                    <input type="url" class="form-control" data-social-input="instagram" value="{{ old('instagram_url', $consultant->instagram_url) }}" placeholder="https://instagram.com/yourhandle">
                </div>
            </div>
        ...
        <div class="consultant-form-card mb-4">
            ...
            <div id="sectionsContainer">
                @foreach($consultant->pageSections as $i => $section)
                    @include('backend.consultant.public-page._section', ['index' => $i, 'section' => $section])
                @endforeach
            </div>
        </div>
    </form>
...
<template id="sectionTemplate">
    @include('backend.consultant.public-page._section', ['index' => '__INDEX__', 'section' => null])
</template>
@endsection

@push('scripts')
<script src="https://cdn.ckeditor.com/ckeditor5/41.4.2/classic/ckeditor.js" defer></script>
<script src="{{ asset('assets/js/vendor-public-page.js') }}?v={{ $consultantPublicPageJsVersion }}" defer></script>
@endpush
