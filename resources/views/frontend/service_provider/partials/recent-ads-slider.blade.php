@php
    ...
    $defaultCategoryLabel = $defaultCategoryLabel ?? 'Services';
@endphp
                    ...
                    @foreach($ads as $ad)
                        @php
                            ...
                            $selectedServiceNames = collect($ad->selected_modules ?? [])
                                ...
                                ->all();
                        @endphp
                        <article class="prod-card recent-ad-card recent-ads-carousel-item vendor-store-recent-ad-card js-ad-modal-trigger"
                            role="button"
                            tabindex="0"
                            @include('frontend.ads.partials.ad-modal-attrs', [
                                'ad' => $ad,
                                'adModalMeta' => $selectedCategoryNames !== [] ? implode(', ', $selectedCategoryNames) : ($defaultCategoryLabel ?? 'Services'),
                            ])
                            data-ad-services="{{ implode(', ', $selectedServiceNames) }}"
                        >
                            <img src="{{ asset($ad->final_image) }}" alt="{{ $ad->title }}" width="768" height="1080" loading="{{ $loop->first ? 'eager' : 'lazy' }}" fetchpriority="{{ $loop->first ? 'high' : 'low' }}" decoding="async">
                            <div class="prod-card-body">
                                <h6 class="mb-1 offer-coupon-title">{{ $ad->title }}</h6>
                                <span class="recent-ad-meta">
                                    <i class="fa-solid fa-layer-group"></i>
                                    {{ ($selectedCategoryNames !== [] ? implode(', ', $selectedCategoryNames) : ($defaultCategoryLabel ?? 'Services')) }} • {{ $ad->created_at?->format('d M Y') ?? 'N/A' }}
                                </span>
                            </div>
                        </article>
                    @endforeach
