...
function layoutAdsGrid(){
    ...
    const firstWidth = items[0].offsetWidth || 320;
    const colUnit = Math.max(200, firstWidth);
    const cols = Math.max(1, Math.floor((containerWidth + gap) / (colUnit + gap)));
    const colHeights = Array(cols).fill(0);
    items.forEach((item)=>{
        const itemWidth = item.offsetWidth || colUnit;
        const span = Math.min(cols, Math.max(1, Math.round((itemWidth + gap) / (colUnit + gap))));
        ...
        const left = bestCol * (colUnit + gap);
