...
<section class="vendor-hero-text-section">
    <div class="container">
        ...
        @if($vendor->hero_sub_heading)
            <p class="lead mb-0 opacity-90" style="@if(!empty($vendor->hero_sub_style)){{ collect($vendor->hero_sub_style)->map(fn($v,$k)=>$k.':'.$v)->implode(';') }}@endif">{!! html_entity_decode($vendor->hero_sub_heading) !!}</p>
        @endif
    </div>
</section>
