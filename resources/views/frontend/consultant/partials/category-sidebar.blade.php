<aside class="vendor-store-category-sidebar">
    <div class="vendor-store-category-sidebar__head">
        <h2 class="vendor-store-category-sidebar__title">Consultant categories</h2>
        <span class="vendor-store-category-sidebar__count">{{ $consultantCategories->count() }} total</span>
    </div>
    <ul class="vendor-store-category-list">
        <li>
            <a href="{{ route('consultant.public-services.index', $consultant->slug) }}"
               class="vendor-store-category-link {{ empty($activeCategory) && empty($activeSubcategory) ? 'is-active' : '' }}">
                All services
            </a>
        </li>
        @forelse($consultantCategories as $category)
            <li class="vendor-store-category-item">
                <a href="{{ route('consultant.public-services.category', [$consultant->slug, $category->id]) }}"
                ...
                @if(($activeCategory?->id ?? null) === $category->id && $category->children->isNotEmpty())
                    <ul class="vendor-store-subcategory-list">
                        @foreach($category->children as $subcategory)
                            <li>
                                <a href="{{ route('consultant.public-services.subcategory', [$consultant->slug, $category->id, $subcategory->id]) }}"
