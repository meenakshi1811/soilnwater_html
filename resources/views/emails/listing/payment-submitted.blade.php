...
@php($subjectLine = $submission->listingTypeLabel().' payment proof submitted')

@section('content')
    <h2 style="margin:0 0 10px;font-size:28px;color:#0f172a;">{{ $submission->listingTypeLabel() }} Payment Proof Submitted</h2>
    <p style="margin:0 0 18px;color:#475569;font-size:14px;">
        A user has submitted payment proof for a paid {{ strtolower($submission->listingTypeLabel()) }}. Please review the attached screenshot and verify the payment in the admin portal to activate it.
    </p>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:separate;border-spacing:0;border:1px solid #dbe5ef;border-radius:10px;overflow:hidden;">
        ...
        <tr>
            <td style="padding:12px 14px;background:#f8fbff;border-bottom:1px solid #dbe5ef;font-weight:700;color:#0f172a;">Type</td>
            <td style="padding:12px 14px;border-bottom:1px solid #dbe5ef;color:#1e293b;">{{ $submission->listingTypeLabel() }}</td>
        </tr>
        <tr>
            <td style="padding:12px 14px;background:#f8fbff;border-bottom:1px solid #dbe5ef;font-weight:700;color:#0f172a;">Title</td>
            <td style="padding:12px 14px;border-bottom:1px solid #dbe5ef;color:#1e293b;">{{ $submission->listingDisplayName() }}</td>
        </tr>
        @if($submission->amount)
            <tr>
                <td style="padding:12px 14px;background:#f8fbff;border-bottom:1px solid #dbe5ef;font-weight:700;color:#0f172a;">Amount</td>
                <td style="padding:12px 14px;border-bottom:1px solid #dbe5ef;color:#1e293b;">₹{{ number_format((float) $submission->amount, 2) }}</td>
            </tr>
        @endif
    ...
    <a href="{{ route('admin.listing-payments.show', $submission) }}" style="display:inline-block;padding:11px 18px;border-radius:8px;background:#12824e;color:#fff;text-decoration:none;font-weight:700;">Review in Admin Portal</a>
@endsection
