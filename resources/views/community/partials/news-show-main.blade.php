...
@if($thumbUrls !== [])
    <div class="news-detail-thumbs">
        @foreach(array_slice($thumbUrls, 0, 4) as $index => $imageUrl)
            <div class="news-detail-thumbs__item">
                <img src="{{ $imageUrl }}" alt="{{ $post->title }} — image {{ $index + 2 }}">
            </div>
        @endforeach
    </div>
@endif
...
<div class="news-detail-body community-post-body community-post-body--scalable"
